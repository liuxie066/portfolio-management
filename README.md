...
```bash
# 手工单账户入口默认不覆盖；只有明确修订时才加 --overwrite
...
它会优先调用服务端 `daily_report_bundle`，用同一个估值快照完成 NAV 预览、日报 payload 和 HTML 数据组装；默认仍会写 HTML 静态产物，但不会写 `nav_history`。只有显式传 `--write-nav --confirm` 才持久化 NAV，且仍默认拒绝覆盖同日记录。`scripts/generate_daily_report_html.py` 只是 renderer，不负责拉数据。
...
写入交给保守的 Python 安装器。`--apply` 前必须先由操作者配置两份 encrypted
credential；安装器只检查名称和文件元数据，不接收或生成 Secret。
