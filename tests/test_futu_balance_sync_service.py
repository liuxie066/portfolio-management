...
def test_futu_cash_never_falls_back_to_non_authoritative_fields():
    ...
    assert provider._cash_balances_from_row({
        "cash": 999,
        "available_funds": 100,
        "withdraw_cash": 90,
        "power": 200,
    }) == {"CNY": None, "USD": None, "HKD": None}
    assert provider._cash_balances_from_row({
        "cash": 999,
        "cn_cash": 0,
        "us_cash": 1,
        "hk_cash": 2,
    }) == {"CNY": 0.0, "USD": 1.0, "HKD": 2.0}
...
def test_authoritative_cash_and_mmf_missing_block_before_write():
    ...
    provider = SimpleNamespace(
        fetch_balances=lambda: FutuBalanceSnapshot(
            cash_by_currency={"CNY": 0, "USD": None, "HKD": 0},
            mmf=0,
            source="futu-openapi",
            account_id=123,
            profile_fingerprint="sha256:redacted",
            cash_source_fields=dict(FutuOpenApiBalanceProvider.CASH_COLUMNS),
            cash_present_by_currency={"CNY": True, "USD": False, "HKD": True},
    ...
    assert "per-currency cash fields are missing: USD" in result["error"]
    assert storage.updates == []
    assert storage.creates == []


def test_authoritative_zero_is_present_and_preserves_per_currency_evidence():
    account_fingerprint = (
        "sha256:"
        "a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3"
    )
    provider = SimpleNamespace(
        fetch_balances=lambda: FutuBalanceSnapshot(
            cash_by_currency={"CNY": 0, "USD": 0, "HKD": 0},
            mmf=0,
            source="futu-openapi",
            account_id=123,
            profile_fingerprint="sha256:redacted",
    ...
    assert result["success"] is True
    assert result["source_metadata"]["cash"] == {
        "mode": "per_currency",
        "present": True,
        "source_fields": {
            "CNY": "cn_cash",
        ...
        "present_by_currency": {"CNY": True, "USD": True, "HKD": True},
    }
